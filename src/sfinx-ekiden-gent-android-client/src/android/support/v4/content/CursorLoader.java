...
public class CursorLoader extends AsyncTaskLoader<Cursor> {
	...
	public Cursor loadInBackground() {
		if (DEBUG) {
			Log.v(TAG, "loadInBackground(URI=" + mUri + ";Projection="
					+ mProjection + ";Selection=" + mSelection
					+ ";SelectionArgs=" + mSelectionArgs + ";SortOrder="
					+ mSortOrder + ")");
		}
		int i = 0;
		for (String proj : mProjection) {
			Log.v(TAG, "Projection[" + i++ + "]" + proj);
		}
		Context context = getContext();
		ContentResolver resolver = context.getContentResolver();
		Log.v(TAG, "ContentResolver: " + resolver.toString());
		Log.v(TAG, "Query " + mUri);
		Cursor cursor = resolver.query(mUri, mProjection, mSelection,
