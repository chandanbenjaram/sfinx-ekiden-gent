...
public class ViewPager extends ViewGroup {
    ...
    void populate() {
        ...
        int lastPos = -1;
        if(DEBUG) {Log.v(TAG,"populating items startPost="+startPos+";endPos="+endPos);}
        for (int i=0; i<mItems.size(); i++) {
            ItemInfo ii = mItems.get(i);
            if(DEBUG) {Log.v(TAG,"populating items for ItemInfo(position=" + ii.position + "");}
            if ((ii.position < startPos || ii.position > endPos) && !ii.scrolling) {
                mItems.remove(i);
                i--;
                mAdapter.destroyItem(this, ii.position, ii.object);
            } else if (lastPos < endPos && ii.position > startPos) {
            	if(DEBUG){Log.v(TAG,"next item is out of range(lastPos("+lastPos+") < endPos("+endPos+") "+
            			"&& (ii.position("+ii.position+") > startPos("+startPos+"))), "+
            			"but we have a gap. Fill it");}
